...
expose_opencode_command() {
  ...
  # Configurar NODE_EXTRA_CA_CERTS para confiar en el certificado CA instalado
  local node_extra_ca_certs=""
  if [[ -f "$CA_CERT_PATH" ]]; then
    node_extra_ca_certs="NODE_EXTRA_CA_CERTS=$CA_CERT_PATH"
    echo "[OK] Certificado CA disponible para Node.js en $CA_CERT_PATH"
  fi

  # Fallback a NODE_TLS_REJECT_UNAUTHORIZED=0 solo si no se pudo instalar el certificado
  local node_tls_reject=""
  if [[ ! -f "$CA_CERT_PATH" ]]; then
    node_tls_reject="NODE_TLS_REJECT_UNAUTHORIZED=0"
    echo "[WARN] Certificado CA no encontrado; usando NODE_TLS_REJECT_UNAUTHORIZED=0 como fallback."
  fi
...
#!/usr/bin/env bash
${node_extra_ca_certs:+$node_extra_ca_certs; export NODE_EXTRA_CA_CERTS; }
${node_tls_reject:+$node_tls_reject; export NODE_TLS_REJECT_UNAUTHORIZED; }
${ld_preload:+$ld_preload; export LD_PRELOAD; }
exec $OPENCODE_BIN "\$@"
EOF_OPENCODE
  chmod +x "$target"
...
# Ruta donde se instalará el certificado CA wildcard
CA_CERT_PATH="/usr/local/share/ca-certificates/cpd-local-wildcard.crt"

install_ca_certificate() {
  echo "[INFO] Instalando certificado CA *.cpd.local ..."

  mkdir -p /usr/local/share/ca-certificates

  cat > "$CA_CERT_PATH" <<'EOF_CERT'
-----BEGIN CERTIFICATE-----
...
fetch_and_update_models() {
  ...
  # Primero intentar con certificados normales; si falla SSL, usar -k como fallback
  local curl_insecure=""
  if [[ -f "$CA_CERT_PATH" ]]; then
    curl_insecure=""
  else
  ...
  local status
  status="$(curl -sSL $curl_insecure --max-time 20 -o "$tmp_json" -w "%{http_code}" \
    -H "Authorization: Bearer $apikey" \
    "$models_url" 2>/dev/null || true)"

  # Si falla por certificado SSL, reintentar con -k
  if [[ "$status" != "200" && -z "$curl_insecure" ]]; then
