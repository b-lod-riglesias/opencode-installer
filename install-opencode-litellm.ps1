...
# =============================================================================
# CERTIFICADO CA (persistente + almacén Windows)
# =============================================================================
function Get-CaCertPath {
    ...
    return Join-Path $certDir "cpd-local-wildcard.crt"
}

function Install-CaCertificate {
    Write-Info "Instalando certificado CA *.cpd.local ..."

    # Guardar certificado en ubicación persistente para Node.js y referencia manual
    $certPath = Get-CaCertPath
    $script:CaCertPem | Out-File -Encoding ascii -FilePath $certPath -Force
    Write-Ok "Certificado CA guardado en $certPath"

    # Intentar almacén de máquina (requiere admin)
    $proc = Start-Process certutil -ArgumentList "-addstore","-f","Root",$certPath -Wait -PassThru -WindowStyle Hidden -ErrorAction SilentlyContinue
    if ($proc.ExitCode -eq 0) {
        Write-Ok "Certificado CA instalado en el almacén de confianza del sistema (Root)."
        return
    }

    # Fallback a almacén de usuario
    Write-Warn "No se pudo instalar a nivel de sistema (código $($proc.ExitCode)). Intentando almacén de usuario ..."
    $proc2 = Start-Process certutil -ArgumentList "-addstore","-f","-user","Root",$certPath -Wait -PassThru -WindowStyle Hidden -ErrorAction SilentlyContinue
    if ($proc2.ExitCode -eq 0) {
        Write-Ok "Certificado CA instalado en el almacén de usuario (Root)."
    } else {
        Write-Warn "No se pudo instalar automáticamente en el almacén de certificados."
        Write-Plain "        Instálalo manualmente: doble clic en $certPath → Instalar certificado → Equipo/Usuario → Raíz de confianza."
    }
}
...
function Set-SslBypassEnvironment {
    Write-Info "Configurando variables de entorno SSL ..."

    $certPath = Get-CaCertPath

    # Primera opción: NODE_EXTRA_CA_CERTS (más seguro, confía solo en nuestro CA)
    ...
    # Fallback: NODE_TLS_REJECT_UNAUTHORIZED=0 solo si el certificado no está en el almacén del sistema
    $certInstalled = $false
    try {
        $store = Get-Item "Cert:\LocalMachine\Root" -ErrorAction SilentlyContinue
        if ($store) {
            $thumbprint = (Get-Content $certPath | Select-String "-----END CERTIFICATE-----" | ForEach-Object { $_ })
            # Comprobación simplificada: si el archivo existe asumimos que certutil lo intentó
            $certInstalled = $true
        }
    } catch { }

    if (-not $certInstalled -and -not (Test-Path "Cert:\LocalMachine\Root")) {
        # Si no somos admin y no podemos verificar, dejamos el fallback activo por seguridad
        [Environment]::SetEnvironmentVariable("NODE_TLS_REJECT_UNAUTHORIZED", "0", "User")
        $env:NODE_TLS_REJECT_UNAUTHORIZED = "0"
        Write-Warn "Certificado CA no confirmado en el almacén del sistema."
        Write-Warn "NODE_TLS_REJECT_UNAUTHORIZED=0 activado como fallback temporal."
    } else {
        # Limpiar fallback si existía de una ejecución anterior
        $existing = [Environment]::GetEnvironmentVariable("NODE_TLS_REJECT_UNAUTHORIZED", "User")
        if ($existing -eq "0") {
            [Environment]::SetEnvironmentVariable("NODE_TLS_REJECT_UNAUTHORIZED", $null, "User")
            Write-Info "NODE_TLS_REJECT_UNAUTHORIZED eliminado (certificado CA confiable activo)."
        }
    }
...
function Invoke-LiteLLMModels {
    ...
    # Intentar primero con verificación de certificado normal
    try {
        $resp = Invoke-RestMethod -Uri "$Url/models" -Headers $headers -TimeoutSec 20 -ErrorAction Stop
        return @{ Success = $true; Response = $resp }
    } catch {
        $errMsg = $_.Exception.Message
        if ($errMsg -notmatch "SSL|certificate|trust|chain|remote|Tls|secure") {
            return @{ Success = $false; Error = $_.Exception.Message }
        }
    }

    # Fallback: reintentar con bypass de certificado
    ...
    try {
        if ($PSVersionTable.PSVersion.Major -ge 6) {
            ...
            # PowerShell 5.1: usar callback global (más seguro que Add-Type reiterado)
            [System.Net.ServicePointManager]::ServerCertificateValidationCallback = { $true }
            $resp = Invoke-RestMethod -Uri "$Url/models" -Headers $headers -TimeoutSec 20 -ErrorAction Stop
            # Restaurar callback para no afectar otras conexiones
